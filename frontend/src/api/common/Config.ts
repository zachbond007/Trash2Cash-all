// export const apiUrl = 'http://192.168.0.101:7189/api/';
export const apiUrl = 'https://trash2cashapi.trash2cash.us/v1/api/';
// export const apiUrl = 'http://localhost:5248/api/';
// export const apiUrl = 'http://yigit-mbp.local:5248/api/';
// export const apiUrl = 'http://yilmaz-mbp.local:5248/api/';
// export const apiUrl = 'http://0.0.0.0:5248/api/';

export const s3BaseUrl =
  //'https://s3-us-east-2.amazonaws.com/trash2cash-s3-bucket/';
  'https://trash2cash-s3-bucket-1.s3.eu-north-1.amazonaws.com/'
